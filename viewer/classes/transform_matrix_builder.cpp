...
TransformMatrix TransformMatrixBuilder::CreateRotationX(float angle_rad) noexcept {
	...
	matrix.SetElement(1, 1, cos_a);
	matrix.SetElement(1, 2, -sin_a);
	matrix.SetElement(2, 1, sin_a);
	matrix.SetElement(2, 2, cos_a);
...
TransformMatrix TransformMatrixBuilder::CreateRotationY(float angle_rad) noexcept {
	...
	matrix.SetElement(0, 0, cos_a);
	matrix.SetElement(0, 2, sin_a);
	matrix.SetElement(2, 0, -sin_a);
	matrix.SetElement(2, 2, cos_a);
...
TransformMatrix TransformMatrixBuilder::CreateRotationZ(float angle_rad) noexcept {
	...
	matrix.SetElement(0, 0, cos_a);
	matrix.SetElement(0, 1, -sin_a);
	matrix.SetElement(1, 0, sin_a);
	matrix.SetElement(1, 1, cos_a);
...
TransformMatrix TransformMatrixBuilder::CreateScaleMatrix(float x, float y, float z) noexcept {
	...
	matrix.SetElement(0, 0, (x != 0.0f) ? x : 1.0f);
	matrix.SetElement(1, 1, (y != 0.0f) ? y : 1.0f);
	matrix.SetElement(2, 2, (z != 0.0f) ? z : 1.0f);
...
TransformMatrix TransformMatrixBuilder::CreateOrthographicMatrix(float left, float right, float bottom, float top, float near, float far) noexcept {
	...
	matrix.SetElement(0, 0, 2.0f / (right - left));
	matrix.SetElement(1, 1, 2.0f / (top - bottom));
	matrix.SetElement(2, 2, -2.0f / (far - near));
	matrix.SetElement(0, 3, -(right + left) / (right - left));
	matrix.SetElement(1, 3, -(top + bottom) / (top - bottom));
