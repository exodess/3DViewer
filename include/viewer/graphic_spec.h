...
namespace viewer {
	...
	*/
	...
	private:
		...
		SSBO ssboMaterial_; ///< Буфер материала фигуры в Smooth Shading шейдерной программе
		SSBO ssboCamera_; ///< Буфер камеры (общий для всех шейдерных программ)
	...
	public:
		...
		/**
		@brief Загрузка данных о вершинах и поверхностях фигуры\n
		Вершины загружаются в VBO буфер видеокарты следующей структурой:
		- Первые 3 float переменные относятся к координате вершины
		- Вторые 3 float переменные относятся к вектору нормали
		Индексы загружаются в EBO буфер плоским массивом
		@param figure Считанная из файла фигура
		*/
		void loadGeometryData(const Figure& figure) noexcept;

		/**
		 * @brief Загружает главные данные по каждой фигуре в буферы,
		 * которые находятся в Ray-tracing шейдерной программе.
		 * @param figures_list Все фигуры, которые в данный момент расположены на схеме
		 */
		void loadFiguresData(const std::vector<Figure>& figures_list) noexcept;
