...
namespace viewer {
	...
	*/
	...
	public:
		...
		/**
		 * @brief Получение всех фигур, которые в данный момент располагаются на сцене
		 * @return Список фигур
		 */
		const std::vector<Figure>& getAllFigure() const noexcept;
		 ...
		 * @return Сумма количества вершин всех фигур на сцене
		 */
		int countVertices() noexcept;

		/**
		* @brief Получение количества поверхностей, одновременно обрабатываемых на сцене
		* @return Сумма количества поверхностей всех фигур на сцене
		*/
		int countSurfaces() noexcept;
